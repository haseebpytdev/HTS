...
@section('admin-content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h1 class="h4 mb-0">Gallery — {{ $group->name }}</h1>
            <small class="text-muted">{{ $group->slug }}</small>
        </div>
        <a href="{{ route('admin.cms.groups.index') }}" class="btn btn-sm btn-outline-secondary">All groups</a>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('admin.groups.gallery.store', $group) }}" enctype="multipart/form-data" class="row g-2 align-items-end">
                @csrf
                <div class="col-md-4">
        ...
        @forelse($group->images as $img)
            <div class="col-md-6 col-lg-4">
                <div class="card border-0 shadow-sm h-100">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.groups.gallery.images.update', [$group, $img]) }}" class="small">
                            @csrf
                            @method('PATCH')
                            <div class="mb-1">
                        ...
                        <form method="POST" action="{{ route('admin.groups.gallery.images.destroy', [$group, $img]) }}" class="mt-2" onsubmit="return confirm('Delete this image?');">
                            @csrf
                            @method('DELETE')
