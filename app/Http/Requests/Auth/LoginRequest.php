    ...
    public function authenticate(): void
    {
        ...
        if ($login !== '') {
            if (filter_var($login, FILTER_VALIDATE_EMAIL)) {
                ...
                } elseif (Auth::guard('customer')->attempt(['email' => $login, 'password' => $password], $remember)) {
                    $this->authenticatedGuard = 'customer';
                }
            } elseif (Schema::hasColumn('users', 'username')) {
                $attempted = true;
                if (Auth::guard('web')->attempt(['username' => $login, 'password' => $password], $remember)) {
                    $this->authenticatedGuard = 'web';
                }
            }
        }

        if ($this->authenticatedGuard === null) {
