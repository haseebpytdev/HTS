...
@section('admin-content')
    <div class="mb-3">
        <h1 class="h4 mb-0">Create Group</h1>
    </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.groups.store') }}" class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" name="departure_date" class="form-control @error('departure_date') is-invalid @enderror" value="{{ old('departure_date') }}">
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="capacity" min="1" class="form-control @error('capacity') is-invalid @enderror" value="{{ old('capacity') }}" required>
                    @error('capacity')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-3">
                    <label class="form-label">Seats left</label>
                    <input type="number" name="seats_left" min="0" class="form-control @error('seats_left') is-invalid @enderror" value="{{ old('seats_left') }}" required>
                    @error('seats_left')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="col-md-6">
                    <label class="form-label">Pricing tiers</label>
                    <textarea name="pricing_tiers" rows="3" class="form-control @error('pricing_tiers') is-invalid @enderror">{{ old('pricing_tiers') }}</textarea>
                    @error('pricing_tiers')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">Airline info</label>
                    <textarea name="airline_info" rows="3" class="form-control @error('airline_info') is-invalid @enderror">{{ old('airline_info') }}</textarea>
                    @error('airline_info')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">Hotel info</label>
                    <textarea name="hotel_info" rows="3" class="form-control @error('hotel_info') is-invalid @enderror">{{ old('hotel_info') }}</textarea>
                ...
                <div class="col-md-2">
                    <div class="form-check mt-2">
                        ...
                        <input type="checkbox" class="form-check-input" id="is_featured" name="is_featured" value="1" @checked((bool) old('is_featured', false))>
