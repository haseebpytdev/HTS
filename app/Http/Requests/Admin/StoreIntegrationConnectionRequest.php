...
class StoreIntegrationConnectionRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            $finalCredentials = $this->normalizeProviderCredentials(
                $normalizedProvider,
            ...
            if (AmadeusSelfServiceProvider::matches($provider)) {
                $clientId = trim((string) ($finalCredentials['client_id'] ?? ''));
                $clientSecret = trim((string) ($finalCredentials['client_secret'] ?? ''));
            ...
            $apiToken = trim((string) ($finalCredentials['api_token'] ?? ''));
            $environment = (string) $this->input('environment', 'sandbox');
            if ($apiToken === '') {
                $validator->errors()->add('credentials.api_token', 'Duffel api_token is required.');
            }

            if ($apiToken !== '' && $environment === 'sandbox' && ! str_starts_with($apiToken, 'duffel_test_')) {
