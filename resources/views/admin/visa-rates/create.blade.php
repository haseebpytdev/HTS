...
@section('title', 'Create Visa Rate')

@section('admin-content')
    <div class="mb-3">
        <h1 class="h4 mb-0">Create Visa Rate</h1>
    </div>

    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.visa-rates.store') }}" class="row g-3">
                @csrf
                <div class="col-md-4">
                    ...
                    <select name="visa_type_id" class="form-select @error('visa_type_id') is-invalid @enderror" required>
                        ...
                        @foreach($visaTypes as $visaType)
                            <option value="{{ $visaType->id }}" @selected((string) old('visa_type_id') === (string) $visaType->id)>{{ $visaType->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 d-flex gap-2">
                    <button type="submit" class="btn btn-primary">Create Visa Rate</button>
                    <a href="{{ route('admin.visa-rates.index') }}" class="btn btn-outline-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </div>
